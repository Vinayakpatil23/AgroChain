...
app.post("/updateOrderStatus", (req, res) => {
  ...
  db.query(fetchOrderQuery, [orderId], (err, results) => {
    ...
    // Determine buyerStatus based on the provided order status
    const buyerStatus =
      status === "Order Cancelled by Buyer" ? "Cancelled" : null;

    // Update the order status and buyer status
    const updateOrderQuery = `
      UPDATE ordering
      SET orderStatus = ?, buyerStatus = ?
      WHERE orderId = ?
    `;
    db.query(updateOrderQuery, [status, buyerStatus, orderId], (err) => {
      if (err) {
        console.error('Error updating order status and buyer status: ' + err.stack);
        return res.status(500).send('Error updating order status and buyer status');
      }
...
const contractAddress = '0x1396B1FF59d69dc511f7C2b97C1C0480d18de37d';  // Replace with your deployed contract address

const paymentContract = new web3.eth.Contract(contractABI, contractAddress);

// Ganache accounts (Farmer and Buyer)

const farmerAccount = '0x99Cd983703a90d6DF5326333Fb7186c0Baf0D09B';    // Replace with the farmer's Ganache address
const buyerAccount = '0x28FF15e5840ED3ef0e1AbDC8332a3331168E88a3'; // Replace with the buyer's Ganache address
